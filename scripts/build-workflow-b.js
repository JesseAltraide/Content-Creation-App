...
addNode({
  parameters: { path: "wf-b-generate-evaluate", httpMethod: "POST", responseMode: "onReceived", options: {} },
  id: "webhook",
...
function buildRevisionRound(roundNum, prevGateIfName, prevSectionName) {
  ...
  const revisePrompt =
    "`Revise this article based on the evaluation feedback below. Handling conflicting or undateable sources: resolve it BEFORE you write, and write the resolution, not the reasoning. Prefer the source that states its own timeframe explicitly over one that does not. If two excerpts cannot both be true and you cannot tell which holds, leave the claim out entirely: a missing figure costs the reader nothing, a paragraph about why you are unsure costs them the article. Never write a sentence about what the excerpts do or do not establish, never give a section a heading about sourcing, and never explain that two details 'should not be conflated'. At most one short caveat in the whole piece, inline, and only where its absence would mislead. You are writing for the reader named in the audience profile, not for a fact-checker reviewing your work. The recommended changes are the evaluator's wording, not fact. They are advisory on structure and emphasis, and are NOT authoritative on any figure, unit, name or date. If following one would state something the excerpts do not support, ignore that part and fix the underlying point another way. Accuracy outranks every recommendation here. Address the flagged sections and unsupported claims specifically - stay grounded only in the excerpts, don't invent anything new to fill gaps.\\n\\nKeep every citation. The article you are revising cites its sources inline as [Source: url] and the revised article must carry every one of them across. Rewrite a sentence and its citation goes with it. Merge two sentences and both citations stay. Add a claim and it gets the same inline citation as everything else. Coming back with fewer citations than you were given is a failed revision no matter how well it reads: the citations are what the reader traces the claims through and what every later stage re-checks against.\\n\\nCurrent article:\\n${$('" +
    prevSectionName +
